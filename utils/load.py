...
def dataset_to_dataframe_v3(filepath, numeric_cols, keep_houses=True):
    dataset = pd.read_csv(filepath)
    ...
    if keep_houses and 'Hogwarts House' in result.columns:
        for col in numeric_cols:
            house_means = result.groupby('Hogwarts House')[col].mean()
            for house, mean in house_means.items():
                mask = (result['Hogwarts House'] == house) & (result[col].isna())
                result.loc[mask, col] = mean
